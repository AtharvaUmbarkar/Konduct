...
const AddCourse = (props) => {
  ...
  return (
    <form method="post" onSubmit={submitAddCourse} id='add-course-instructor' className='flex flex-row items-center h-8 text-sm space-x-1 my-3 mb-2'>
      <select title='course' name="course" value={course} onChange={changeAddCourse} className='bg-tertiary outline-none border border-accent rounded-sm shadow-none w-48 focus:border-secondary p-1 h-full'>
        <option value='' form='add-course-instructor'></option>
        {props.courses.map((course, idx) => {
          return (
            <option key={course.id} value={course.id} form='add-course-instructor' className=''>{course.id}</option>
          )
        })}
      </select>
