    ...
    <div className='grow flex flex-col justify-start items-center'>
      ...
      <div className='w-4/5 grid grid-cols-3 gap-2 m-4 mt-8'>
        {userCourses.map((course, index) => (
          <NavLink to={`/instructor/course/${course.id}`} key={course.id} className='text-xl h-20 w-full bg-tertiary text-accent hover:text-secondary hover:bg-opacity-70 grid place-items-center rounded-sm'>
