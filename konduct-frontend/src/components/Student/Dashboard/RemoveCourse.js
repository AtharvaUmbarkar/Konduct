...
const RemoveCourse = (props) => {
  ...
  return (
    <form method="post" onSubmit={submitRemoveCourse} id='remove-course-student' className='flex flex-row items-center h-8 text-sm space-x-1'>
      <select title='course' name="course" value={course} onChange={changeRemoveCourse} className='bg-tertiary outline-none border border-accent rounded-sm shadow-none w-48 focus:border-secondary p-1 h-full'>
        <option value='' form='remove-course-student'></option>
        {props.courses.map((course, idx) => {
          return (
            <option key={course.id} value={course.id} form='remove-course-student' className=''>{course.id}</option>
          )
        })}
      </select>
